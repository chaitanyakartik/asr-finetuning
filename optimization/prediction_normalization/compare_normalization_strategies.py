"""
Compare two normalization strategies:
...
def apply_normalization(predictions, normalizer, strategy_name):
    ...
    # Calculate CER separately (jiwer doesn't provide it in WordOutput)
...
def show_examples(conservative_result, aggressive_result, num_examples=10):
    """Show example differences between strategies."""
    print(f"\n{'='*80}")
    print(f"Example Differences (first {num_examples})")
    print(f"{'='*80}\n")
    
    cons_preds = conservative_result['normalized_predictions']
    agg_preds = aggressive_result['normalized_predictions']
    
    shown = 0
    for i, (cons, agg) in enumerate(zip(cons_preds, agg_preds)):
        cons_pred = cons['prediction_normalized']
        agg_pred = agg['prediction_normalized']
        
        if cons_pred != agg_pred and shown < num_examples:
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python compare_normalization_strategies.py <predictions.json>")
        sys.exit(1)
    ...
    print(f"  • Style Guide: {cons_wer - style_wer:+.2f}% WER change
        print(f"✅ Conservative is BETTER by {abs(wer_diff):.2f}% absolute WER")
    else:
    ...
    if best_strategy == 'Conservative':
        ...
        print(f"✨ Aggressive is {wer_diff:.2f}% better")
        printstyle_guide': {
            ...
            'wer_improvements': {
                'aggressive_vs_conservative': cons_wer - agg_wer,
                'style_guide_vs_conservative': cons_wer - style_wer
            }
        print(f"⚠️  Conservative is {abs(wer_diff):.2f}% better")
        print("    → Aggressive normalization is HURTING performance!")
        print("    → Recommendation: Use conservative approach")
    
    # Show examples
    show_examples(conservative_result, aggressive_result, num_examples=15)
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            ...
            'recommendation': 'conservative' if wer_diff < 0.5 else 'aggressive'
