...
OUTPUT_MANIFEST = "/mnt/data/asr-finetuning/data/training/v3/mixed_kn_en_hi_27.5_27.5_45.json"

# --- RATIO CONFIGURATION ---
# Base: Kannada (27.5% of total)
# Target: English = 27.5%, Hindi = 45%
# Calculations relative to Kannada:
# EN = (27.5 / 27.5) * KN_Duration = 1.0 * KN_Duration
# HI = (45.0 / 27.5) * KN_Duration = 1.636363 * KN_Duration
EN_RATIO = 1.0
HI_RATIO = 1.63636363636

def create_precise_mix():
    kn_entries = []
    
    # 1. Load all Kannada entries (The 27.5% anchor)
    ...
    # 2. Set targets relative to the full Kannada set
    ...
    def sample_dataset(path, target_seconds, lang_label):
        ...
        if current_seconds < target_seconds:
            print(f"⚠️ Warning: Not enough {lang_label} data. "
    ...
    print(f"--- 📊 Precision Mix Report (27.5 / 27.5 / 45) ---")
    print(f"KN (27.5%): {to_hrs(total_kn_seconds):.2f} hrs | Actual: {(total_kn_seconds/total_sec)*100:.2f}%")
    print(f"EN (27.5%): {to_hrs(actual_en_seconds):.2f} hrs | Actual: {(actual_en_seconds/total_sec)*100:.2f}%")
    print(f"HI (45.0%): {to_hrs(actual_hi_seconds):.2f} hrs | Actual: {(actual_hi_seconds/total_sec)*100:.2f}%")
