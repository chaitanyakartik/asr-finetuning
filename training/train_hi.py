...
HI_TOK_DIR = os.path.join(PROJECT_ROOT, "training/tokenizers/hi_master_v1536")  # NEW
...
def run_training(args):
    ...
    # ---------------------------------------------------------
    if args.phase == 0:
        # Phase 0: Add Hindi to existing EN+KN model (3-Language Surgery)
        if not args.base_model:
            raise ValueError("Phase 0 requires --base_model (your trained EN+KN model)")
        
        if local_rank == 0: 
            logging.info("PHASE 0: Adding Hindi Tokenizer to EN+KN Model (3-Language Surgery)")
            logging.info(f"Loading base model: {args.base_model}")
        
        # Load your trained EN+KN model
        model = EncDecHybridRNNTCTCBPEModel.restore_from(args.base_model)
        
        # Create new 3-language tokenizer config
        ...
        new_tokenizer_cfg.langs['hi'] = OmegaConf.create({'dir': HI_TOK_DIR, 'type': 'bpe'})  # NEW
        
        if local_rank == 0:
            logging.info("New tokenizer config:")
            logging.info(f"  EN: 1024 tokens")
            logging.info(f"  KN: 3000 tokens")
            logging.info(f"  HI: 1536 tokens")
            logging.info(f"  Total: 5560 tokens")
        
        # Apply tokenizer surgery - this reinitializes the decoder
        model.change_vocabulary(new_tokenizer_dir=new_tokenizer_cfg, new_tokenizer_type="agg")
        
        if local_rank == 0: logging.info("Tokenizer surgery complete. Decoder reinitialized.")
        
        # FIX: Kill SpecAugment & Dropout for Phase 0 stability
        ...
        # STRATEGY: Freeze Encoder Completely (preserve EN+KN acoustic knowledge)
        model.encoder.freeze()
        if local_rank == 0: 
            logging.info("Encoder: FULLY FROZEN (preserving EN+KN acoustic knowledge)")
            logging.info("Decoder: Training from scratch with 3 languages")
    
    elif args.phase == 1:
        # Phase 1: Fresh Start (Surgery) - Original 2-language
    ...
    if hasattr(model, 'loss'):
        if args.phase in [0, 1]:  # Phase 0 same as Phase 1
            # Phase 0/1: Zero CTC
    ...
    if hasattr(model, 'spec_augmentation') and args.phase in [0, 1]:
        model.spec_augmentation = None # Disable for stability in Phase 0/1
    ...
    # Validation - Updated for 3 languages if Phase 0+
    if args.phase == 0:
        val_files = [
            ...
            os.path.join(PROJECT_ROOT, "evaluation/benchmarking/data/v1/hi_clean_read.json")  # Add this file
        ]
        for v in val_files: 
            lang = "en" if "en_" in v else ("kn" if "kn_" in v else "hi")
            ensure_manifest_tags(v, lang, local_rank)
    else:
        val_files = [
            os.path.join(PROJECT_ROOT, "evaluation/benchmarking/data/v1/en_clean_read.json"),
            os.path.join(PROJECT_ROOT, "evaluation/benchmarking/data/v1/kn_clean_read.json")
    ...
    # Update project name for 3-language model
    project_name = "kannada-asr-curriculum"
    ...
    exp_config = exp_manager.ExpManagerConfig(
        ...
        wandb_logger_kwargs={"name": f"{args.exp_name}_p{args.phase}", "project": project_name}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--phase", type=int, required=True, choices=[0, 1, 2, 3, 4], help="Curriculum Phase (0=Add Hindi to EN+KN)")
    parser.add_argument("--exp_name", type=str, required=True)
    parser.add_argument("--train_manifest", type=str, required=True)
    parser.add_argument("--base_model", type=str, help="Required for Phase 0,2,3,4")
