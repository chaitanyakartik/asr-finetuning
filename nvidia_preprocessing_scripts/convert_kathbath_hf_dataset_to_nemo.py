...
def infer_dataset_segments(batch):
    ...
    """
    segments = []
    segment, path = os.path.split(batch['audio_filepath']['path'])
...
def build_map_dataset_to_nemo_func(cfg: HFDatasetConversionConfig, basedir):
    ...
    def map_dataset_to_nemo(batch):
        ...
        # print(batch)
        if cfg.streaming:
            batch['audio_filepath'] = batch['audio_filepath']['path'].split("::")[0].replace("zip://", "")
        else:
            segments = infer_dataset_segments(batch)
            audio_filepath = os.path.join(*segments)
            # batch['duration']=batch['audio_filepath']['duration']
            batch['sampling_rate']=batch['audio_filepath']['sampling_rate']
            batch['array']=batch['audio_filepath']['array']
        ...
        soundfile.write(audio_filepath, batch['array'], samplerate=cfg.sampling_rate, format='wav')

        # batch['duration'] = librosa.get_duration(y=batch['audio_filepath'], sr=batch['sampling_rate'])
        return batch
...
##### OG
    ...
    #         del sample['audio_filepath']['array']#['audio']
    #         del sample['array']#['audio']
            
    #         sample['audio_filepath']=sample['audio_filepath']['path']

    #         if 'file' in sample:
    #             del sample['file']
    #         manifest_f.write(f"{json.dumps(sample, ensure_ascii=cfg.ensure_ascii)}\n")
###### as is futures
    # def process_sample(sample, cfg):
    # # remove large components from sample
    #     del sample['audio_filepath']['array']
    #     del sample['array']
    ...
    #     return json.dumps(sample, ensure_ascii=cfg.ensure_ascii)
    ...
    #             )
    #         ):
    #             futures.append(executor.submit(process_sample, sample, cfg))
            
    #         for future in concurrent.futures.as_completed(futures):
    #             manifest_f.write(f"{future.result()}\n")          
###### used map over iter to speed up way faster 800it/s
    ...
    processed_dataset = dataset#dataset.map(process_sample)

    # Save the processed dataset to a manifest file
    
    with open(manifest_filepath, 'w') as manifest_f:
        for sample in tqdm.tqdm(processed_dataset, desc=f'Processing {cfg.path} (split : {cfg.split}):', total=len(processed_dataset), unit=' samples'):
            try:
                del sample['audio_filepath']['array']
                del sample['array']
                sample['audio_filepath'] = sample['audio_filepath']['path']
...
def process_dataset(dataset: IterableDataset, cfg: HFDatasetConversionConfig):
    ...
    dataset = dataset.cast_column("audio_filepath", Audio(cfg.sampling_rate, mono=True))
